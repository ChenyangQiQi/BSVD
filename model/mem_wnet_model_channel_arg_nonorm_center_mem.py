...
class InputCvBlock(nn.Module):
    ...
    def forward(self, x, end=False):
        ...
        if x is None: 
            return None
        elif torch.count_nonzero(x) == 0:
            return torch.zeros((self.n, self.out_channels, self.h, self.w), device=torch.device('cuda'))
        else:
...
class UNet(nn.Module):
    ...
    def forward(self, input_seq):
        ...
        with torch.no_grad():
            ...
            end_out = self.feedin_one_element(torch.zeros((1, c, h, w), device=torch.device('cuda')))
