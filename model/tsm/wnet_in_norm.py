...
class CvBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.b1 = nn.InstanceNorm2d(out_ch)
        self.relu1 = nn.ReLU(inplace=True)
        self.c2 = nn.Conv2d(out_ch, out_ch, kernel_size=3,
                            padding=1, bias=False)
        self.b2 = nn.InstanceNorm2d(out_ch)
...
class DenBlock(nn.Module):
    ...
    def forward(self, in1):
        ...
        x = self.outc(x0+x1, in1)

        return x

class SingleUNet(nn.Module):
    def __init__(self, channel=4, mid_channel=32, channels1=[32,64,128]):
...
class UNet(nn.Module):
    def __init__(self, channel=4, mid_channel=32, channels1=[32,64,128], channels2=[32,64,128]):
