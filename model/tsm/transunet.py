...
class DenBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, mid_channel=32, residual_num=1):
        ...
        for i in range(residual_num):
            self.resblock_layers.append(TransBlock(mid_channel * 8))
        self.reset_params()
    ...
    def forward(self, in1):
        ...
        x2 = self.downc1(x1)
        b, c, h, w = x2.size()
        x2 = torch.reshape(x2, [b, c, h*w]).permute(0,2,1)
        for resblock in self.resblock_layers:
            residual = resblock(x2) 
            x2 = x2 + residual
        x2 = torch.reshape(x2.permute(0,2,1), [b, c, h, w])
