...
# Use zeros to mark ending may bring bugs. It is possible features are zeros.
#%%
...
class MemShiftConv(nn.Module):
    ...
    def forward(self, input_right, verbose=False):
        ...
        # Case2: Center is not None, but input_right is None
            ...
            # if 
            # assert torch.count_nonzero(self.center) == 0
            # return_ze
            if verbose: print("%f+%f+none = 0"%(torch.mean(self.left_fold_2fold), torch.mean(self.center)))
            self.left_fold_2fold = self.center[:, self.fold:2*self.fold, :, :]
            self.center = input_right
            # n, c, h, w = self.left_fold_2fold.shape
            return torch.zeros((self.n, self.c, self.h, self.w), device=torch.device('cuda'))
            if verbose: print("%f+%f+none = 0"%(torch.mean(self.left_fold_2fold), torch.mean(self.center)))
...
class InputCvBlock(nn.Module):
    ...
    def forward(self, x, end=False):
        ...
        if x is None and not end: 
            return None, False
        elif end:
            return torch.zeros((self.n, self.out_channels, self.h, self.w), device=torch.device('cuda')), True
        else:
            return self.convblock(x), False
...
class DownBlock(nn.Module):
    ...
    def forward(self, x, end=False):
        ...
        end_memconv = False
        if end :
            x = torch.zeros((self.n, self.out_channels, self.h//2, self.w//2), device=torch.device('cuda'))
            end_memconv = True
        elif x is not None:
            x = self.convblock(x)
            end_memconv = False
...
class OutputCvBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, interm_ch):
        ...
        self.out_channels = out_ch
    def forward(self, x, in1, end= False):
        if x is None: return None
        if x is not None:
            self.n, self.in_channels, self.h, self.w = x.size()
        if end:
            out = torch.zeros((self.n, self.out_channels, self.h, self.w), device=torch.device('cuda'))
        else:
...
class DenBlock(nn.Module):
    ...
    def forward(self, in1, end =False):
        '''Args:
        ...
        self.skip1.push(in1)
        # import pdb; pdb.set_trace()
        x0, end0 = self.inc(in1, end)
        self.skip2.push(x0)
        
        # Downsampling
        x1, end1 = self.downc0(x0, end0)
        self.skip3.push(x1)
        
        x2, end2 = self.downc1(x1, end1)
        # None
        # Upsampling
        x2, end3 = self.upc2(x2, end2) # out_seq=N,N,N
        # self.upc2.memconv.c1.center = None
        # if x2 is None: return None
        x1, end4 = self.upc1(self.none_add(x2, self.skip3.pop(x2)), end3)
        # Estimation
        # import pdb; pdb.set_trace()
        # if x1 is None: return None
        x, end5 = self.outc(self.none_add(x1, self.skip2.pop(x1)), self.skip1.pop(x1), end4)
...
class UNet(nn.Module):
    ...
    def feedin_one_element(self, x, end=False):
        x, end1 = self.temp1(x, end)
        x       = self.temp2(x, end1)
        return x
    ...
    def forward(self, input_seq):
        ...
        with torch.no_grad():
            ...
            end_out = self.feedin_one_element(None, end=True)
