...
class SlimCNNMemConv(nn.Module):
    ...
    def load_from(self, path="./logs/1105_crvd_slimcnn_batch8_lr_1e-4_seq7/ckpt.pth"):
        ckpt = torch.load(path)
        print("load from %s"%path)
        ckpt_state = ckpt['state_dict']
        # print(add_net.inc.convblock[0].weight)
        # previous_weight = self.inc.convblock[0].weight
        ...
        self.outc.c1.op.conv.weight  = torch.nn.Parameter(ckpt_state['base_model.outc.c1.net.weight'])
        self.outc.c2.op.conv.weight  = torch.nn.Parameter(ckpt_state['base_model.outc.c2.net.weight'])
...
#%% 
ckpt = torch.load("../logs/1108_crvd_slimcnn_skip_batch8_lr_1e-4_seq7/ckpt.pth")
ckpt_state = ckpt['state_dict']
#%%
# print(add_net.inc.convblock[0].weight)
...
add_net.midc.c1.op.conv.weight      =   torch.nn.Parameter(ckpt_state['base_model.midc.c1.net.weight'])
add_net.midc.c2.op.conv.weight      =   torch.nn.Parameter(ckpt_state['base_model.midc.c2.net.weight'])
add_net.outc.convblock[0].weight    =   torch.nn.Parameter(ckpt_state['base_model.outc.convblock.0.weight'])
add_net.outc.out_conv.weight        =   torch.nn.Parameter(ckpt_state['base_model.outc.out_conv.weight'])
